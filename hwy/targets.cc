...
int64_t DetectTargets() {
  ...
  if (has_xsave && has_osxsave) {
    ...
    const bool have_avx3_xsave_support =
        detail::IsMacOs12_2OrLater() &&
        detail::HasCpuFeature("hw.optional.avx512f");
#endif
...
inline int64_t& SupportedTargetsForTest() {
  static int64_t supported_targets_for_test_ = 0;
  return supported_targets_for_test_;
}

// Mask of targets disabled at runtime with DisableTargets.
inline int64_t& SupportedMask() {
  static auto supported_mask_ = LimitsMax<int64_t>();
  return supported_mask_;
}
}  // namespace detail
