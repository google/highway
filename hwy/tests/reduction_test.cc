...
struct TestSumOfLanes {
  ...
  HWY_NOINLINE void SignedEvenLengthVectorTests(Simd<T, N, P> d) {
    ...
    HWY_ASSERT_EQ(static_cast<T>(-pairs), SumOfLanesVal(d, v));
    ...
    HWY_ASSERT_EQ(static_cast<T>(pairs * 2), SumOfLanesVal(d, v));
  }
  ...
  HWY_NOINLINE void operator()(T /*unused*/, D d) {
    ...
    HWY_ASSERT_EQ(T(sum), SumOfLanesVal(d, Load(d, in_lanes.get())));
    ...
    HWY_ASSERT_EQ(T(sum), SumOfLanesVal(d, Iota(d, 0)));
