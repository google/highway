  ...
  static bool ReadBytes(int fd, ssize_t size, void* to) {
    ...
    for (int retry = 0; retry < 10; ++retry) {
      ...
      if (HWY_UNLIKELY(bytes_read <= 0)) {
        ...
        HWY_WARN("perf read() failed, errno %d\n", errno);
        return false;
      }
      pos += bytes_read;
      HWY_ASSERT(pos <= size);
    ...
    HWY_WARN("perf read() wanted %d bytes, got %d\n", static_cast<int>(size),
             static_cast<int>(pos));
    return false;
  }
...
#pragma pack(pop)
  ...
  // Returns negative on error, or fraction.
  static double ReadEventsAndExtrapolate(int fd, size_t num_events,
                                         double* HWY_RESTRICT values) {
    Buf buf;
    const ssize_t want_bytes =
        static_cast<ssize_t>(24 + num_events * sizeof(uint64_t));
    if (HWY_UNLIKELY(!ReadBytes(fd, want_bytes, &buf))) return -1.0;

    HWY_ASSERT(num_events == buf.num_events);
    HWY_ASSERT(buf.time_running <= buf.time_enabled);

    // If this group was not scheduled yet, return zeros.
    if (HWY_UNLIKELY(buf.time_running == 0)) {
      for (size_t i = 0; i < num_events; ++i) values[i] = 0.0;
      return 0.0;
    }

    // Compute coverage and extrapolate for each value.
    const double fraction = static_cast<double>(buf.time_running) /
                            static_cast<double>(buf.time_enabled);
    HWY_ASSERT(0.0 < fraction && fraction <= 1.0);
    const double extrapolate = 1.0 / fraction;
 ...
 public:
  bool Init(BitSet64& valid) {
    // Already called and succeeded.
    ...
    for (const CounterConfig& config : AllCounterConfigs()) {
      ...
      if (HWY_UNLIKELY(fd < 0)) {
        HWY_WARN("perf_event_open %d errno %d for counter %s\n", fd, errno,
        ...
        valid.Set(static_cast<size_t>(config.c));
        fds_.push_back(fd);
      }
    }
    ...
    HWY_ASSERT(num_valid == valid.Count());
  ...
  double Read(BitSet64& valid, double* values) {
    if (HWY_UNLIKELY(fds_.empty())) return -1.0;
    ...
    for (const Group& group : groups_) {
      const double fraction =
          ReadEventsAndExtrapolate(group.leader_fd, group.num_events, pos);
      if (HWY_UNLIKELY(fraction < 0.0)) return -1.0;
 ...
 private:
  std::vector<int> fds_;  // one per valid counter
...
double PerfCounters::Read() { return -1.0; }
size_t PerfCounters::IndexForCounter(Counter) { return 0; }
#endif  // HWY_OS_LINUX || HWY_IDE

BitSet64& PerfCounters::Valid() {
