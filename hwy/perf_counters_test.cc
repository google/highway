...
void ReadAndPrint(uint64_t r, double* values) {
  ...
  const double min_coverage = counters.Read();
  ...
  fprintf(stderr, "r: %d, coverage %f, overhead %.1f ns\n", static_cast<int>(r),
          min_coverage, elapsed_ns);

  if (min_coverage >= 0.0) {
    counters.Foreach([&values](double val, PerfCounters::Counter c) {
...
TEST(PerfCountersTest, TestMem) {
  ...
  HWY_ASSERT(values[PerfCounters::kCacheRefs] == 0.0 ||
             values[PerfCounters::kCacheRefs] > 1E5);  // 172K..66M
...
TEST(PerfCountersTest, RunBranches) {
  ...
  // for debug/sanitizer builds.
  HWY_ASSERT(values[PerfCounters::kRefCycles] == 0.0 ||
             values[PerfCounters::kRefCycles] > 1E4);  // 34K..18M
  HWY_ASSERT(values[PerfCounters::kInstructions] == 0.0 ||
  ...
  HWY_ASSERT(values[PerfCounters::kBranchMispredicts] == 0 ||
             values[PerfCounters::kBranchMispredicts] > 100.0);  // 5K
  ...
  HWY_ASSERT(values[PerfCounters::kBusCycles] < 1E9);     // 1M..190M
