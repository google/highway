...
//
// AutoFDO compiler fission line offset constants.
constexpr int kSequentialTinyOffset = 100000;
constexpr int kSequentialMediumOffset = 200000;
constexpr int kSequentialHugeOffset = 300000;
constexpr int kRandomTinyOffset = 400000;
constexpr int kRandomMediumOffset = 500000;
constexpr int kRandomHugeOffset = 600000;
constexpr int kAutoTinyOffset = 700000;
constexpr int kAutoMediumOffset = 800000;
constexpr int kAutoHugeOffset = 900000;
...
inline void DispatchWorkloadFission(size_t total_elements,
                                    Executor&& exec) {
  ...
  if constexpr (Hint == PrefetchTuningHint::kSequential) {
    if (total_elements <= kTinyThreshold) {
      exec.template operator()<kSequentialTinyOffset>(false);
    } else if (total_elements <= kMediumThreshold) {
      exec.template operator()<kSequentialMediumOffset>(false);
    } else {
      exec.template operator()<kSequentialHugeOffset>(false);
    }
  } else if constexpr (Hint == PrefetchTuningHint::kRandom) {
    if (total_elements <= kTinyThreshold) {
      exec.template operator()<kRandomTinyOffset>(false);
    } else if (total_elements <= kMediumThreshold) {
      exec.template operator()<kRandomMediumOffset>(false);
    } else {
      exec.template operator()<kRandomHugeOffset>(false);
    }
  } else {
    // kAuto fallback
    if (total_elements <= kTinyThreshold) {
      exec.template operator()<kAutoTinyOffset>(false);
    } else if (total_elements <= kMediumThreshold) {
      exec.template operator()<kAutoMediumOffset>(false);
    } else {
      exec.template operator()<kAutoHugeOffset>(false);
    }
  }
}
...
template <PrefetchTuningHint Hint, typename Policy, typename LoopRunner>
inline void DispatchTunedWorkload(size_t total_elements,
