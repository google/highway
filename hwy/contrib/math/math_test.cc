      ...
      for (UintT base_mant_frac_bits = 1;
        ...
        for (int exp_of_pow_exp = -4; exp_of_pow_exp <= 4; exp_of_pow_exp++) {
          ...
          for (UintT pow_exp_mant_frac_bits = 1;
...
#if HWY_TARGET <= HWY_NEON_WITHOUT_AES && HWY_ARCH_ARM_V7
            if ((std::abs(pow_exp) < 1e-37f) || (std::abs(expected0) < 1e-37f) ||
            ...
            CheckPowResult(d, expected0, actual0, base, pow_exp, max_ulp, kMaxErrorUlp);

            const T actual1 = GetLane(CallPow(d, v_neg_base, Set(d, int_pow_exp)));
            CheckPowResult(d, expected1, actual1, -base, int_pow_exp, max_ulp, kMaxErrorUlp);
          }
        }
      }
    }
