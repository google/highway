...
void BuildDenseTable(AnsDenseTable& table, const uint32_t freqs[256],
                     uint32_t single_sym) {
  hwy::ZeroBytes(table.data(), table.size() * sizeof(table[0]));
  uint32_t start = 0;
...
HWY_CONTRIB_DLLEXPORT bool Ans32DecodePayloadScalar(const uint8_t* payload,
  ...
  for (int lane = 0; lane < 16; ++lane) {
    state[lane] = LoadLE32(payload + lane * 4);
    state[lane + 16] =
        LoadLE32(payload + static_cast<size_t>(lane) * 4 + cursor_rev);
  }
  ...
  for (;;) {
    ...
    for (int lane = 0; lane < 16; ++lane) {
      if (state[lane] < kAnsWordL) {
        ...
        state[lane] =
            (state[lane] << kAnsWordLBits) | LoadLE16(payload + cursor_fwd);
        cursor_fwd += 2;
      }
    }
    for (int lane = 16; lane < 32; ++lane) {
      if (state[lane] < kAnsWordL) {
        ...
        state[lane] =
            (state[lane] << kAnsWordLBits) | LoadLE16(payload + cursor_rev - 2);
        cursor_rev -= 2;
      }
    }
  }
