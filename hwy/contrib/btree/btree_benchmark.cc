...
void RunBenchmarkSuite(size_t num_keys) {
  ...
  printf("Build Time (Sorted Unique Keys, Step = 10, 100%% Fill Ratio):\n");
  ...
  printf(
      "\nMemory Footprint (100%% Fill Bulk-Loaded State, Step = 10 Keys, "
      "TCMalloc Measured):\n");
  ...
  printf(
      "\nPoint Lookup Latency (1M Uniform Random Queries in [0, 10*N], ~10%% "
      "Hit / ~90%% Miss, 100%% Bulk-Loaded Tree):\n");
  ...
  printf(
      "\nLowerBound Range Query Latency (1M Uniform Random Targets in [0, "
      "10*N], 100%% Bulk-Loaded Tree):\n");
  printf("  std::set             : %6.2f ns/op (%6.2f Mops/s)\n", std_lb_ns,
         1000.0 / std_lb_ns);
  printf("  absl::btree_set      : %6.2f ns/op (%6.2f Mops/s)\n", absl_lb_ns,
         1000.0 / absl_lb_ns);
  ...
  printf(
      "\nBatch Point Lookup (1M Uniform Random Queries in [0, 10*N], 8-way "
      "Pipelined Prefetch):\n");
  ...
  printf(
      "\nBatch LowerBound Query (1M Uniform Random Targets in [0, 10*N], 8-way "
      "Pipelined Prefetch):\n");
  printf("  hwy::BTreeSet (Serial)        : %6.2f ns/op (%6.2f Mops/s)\n",
  ...
  if (compact_del_bytes <= absl_del_bytes) {
    ...
    printf(
        "  hwy::CompactBTreeSet : %6.2f ns/op (%5.1f B/key, %5.2f MB) -> "
        "%.2fx vs absl (%.1f%% larger due to chunk pool)\n",
        compact_erase_ns,
        static_cast<double>(compact_del_bytes) / compact_dyn_set.size(),
        compact_del_bytes / (1024.0 * 1024.0), absl_erase_ns / compact_erase_ns,
        100.0 *
...
void RunMapBenchmarkSuite(size_t num_keys) {
  ...
  printf(
      "\nPoint Lookup Latency (1M Uniform Random Queries in [0, 10*N], ~10%% "
      "Hit / ~90%% Miss, 100%% Bulk-Loaded Map):\n");
  ...
  // 5. Batch Value Lookups (FindValueBatch / LookupBatch - 8-way pipelined
  // prefetch)
  ...
  printf(
      "\nBatch Value Lookup (1M Uniform Random Queries in [0, 10*N], 8-way "
      "Pipelined Prefetch):\n");
  ...
  const double mi_compact_0 = hwy::platform::Now();
  for (size_t i = 0; i < kNumMutations; ++i) {
    compact_dyn_map[mut_keys[i]] = mut_vals[i];
  }
  const double mi_compact_1 = hwy::platform::Now();
  ...
  printf(
      ...
      100.0 * (1.0 - static_cast<double>(compact_dyn_map_bytes) /
                         absl_dyn_map_bytes));
  ...
  if (kNumIncremental > 0) {
    ...
    const double inc_compact_0 = hwy::platform::Now();
    for (size_t i = 0; i < kNumIncremental; ++i) {
      compact_prebuilt_map[inc_keys[i]] = inc_vals[i];
    }
    const double inc_compact_1 = hwy::platform::Now();
