...
class CallerAccumulator {
 public:
  ...
  void Add(size_t tasks, size_t workers, bool is_root, timer::Ticks wait_before,
    ...
    calls_++;
    root_ += is_root;
    workers_ = static_cast<uint32_t>(workers_ + workers);
    tasks_ = static_cast<uint32_t>(tasks_ + tasks);
  ...
  void PrintAndReset(const char* caller, size_t active_clusters) {
    ...
    const double pc_root =
        static_cast<double>(root_) / static_cast<double>(calls_) * 100.0;
    ...
    printf(
        "%40s: %7u x (%3.0f%%) %2zu clusters, %4.1f workers @ %5.1f tasks, "
        "%5.0f us wait, %6.1E us run (task len %6.1E us), total %6.2f s\n",
        caller, calls_, pc_root, active_clusters, avg_workers,
        avg_tasks_per_worker, avg_wait_before * 1E6, avg_elapsed * 1E6,
        task_len * 1E6, elapsed);
...
static_assert(sizeof(CallerAccumulator) == 32, "");
...
class Tasks {
 ...
 public:
  ...
  HWY_POOL_PROFILE void WorkerRun(Worker* worker, const Shared& shared,
                                  Stats& stats) const {
 ...
 private:
  ...
  HWY_POOL_PROFILE void WorkerRunDynamic(Worker* worker, const Shared& shared,
                                         Stats& stats) const {
