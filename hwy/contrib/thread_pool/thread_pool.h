...
template <class Func>
class alignas(8) CallWithBarrier {
 public:
...
struct alignas(8) PoolConfig {  // 8 bytes
  ...
  template <class Func>
  void CallWithSpinAndBarrier(Func&& func) {
    CallWithSpin(spin_type,
                 CallWithBarrier<Func>(barrier_type, std::forward<Func>(func)));
  }
...
class alignas(HWY_ALIGNMENT) ThreadPool {
 public:
  ...
  void SetWaitMode(PoolWaitMode mode) {
    ...
    shared_.main_config.CallWithSpinAndBarrier(
   ...
   public:
    ...
    void operator()() {
      ...
      while (!shared_->waiter.WantStop()) {
        ...
        worker_config_.CallWithSpinAndBarrier(*this);
      }
    }
