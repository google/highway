 ...
 *
 ************************************************************************************
 ** Figure 5.1: Signed division by run-time invariant divisor, rounded toward
 * zero
 ************************************************************************************
...
#if defined(HIGHWAY_HWY_CONTRIB_INTDIV_INTDIV_INL_H_) == \
...
#if !HWY_HAVE_DIV128 || HWY_TARGET_IS_NEON || HWY_TARGET == HWY_PPC8 || \
    HWY_TARGET == HWY_VSX
...
HWY_INLINE constexpr DivisorParamsU<T> ComputeDivisorParams(T divisor) {
  ...
  const uint32_t m =
      ((static_cast<uint32_t>(two_l - divisor) << 8) / divisor) + 1u;
...
HWY_INLINE constexpr DivisorParamsU<T> ComputeDivisorParams(T divisor) {
  ...
  const uint64_t tmp =
      ((static_cast<uint64_t>(two_l - divisor) << 16) / divisor) + 1u;
...
HWY_INLINE
    ...
    constexpr
#endif
    DivisorParamsU<T> ComputeDivisorParams(T divisor) {
  HWY_DASSERT(divisor != 0);
  if (HWY_UNLIKELY(divisor == 0)) {
    HWY_ABORT("intdiv: division by zero in ComputeDivisorParams");
  }
  ...
  const uint64_t two_l_minus_d =
      (l < 64) ? ((1ULL << l) - divisor) : (0 - divisor);
...
HWY_INLINE constexpr DivisorParamsS<T> ComputeDivisorParams(T divisor) {
  ...
  const UT abs_d = divisor < 0
                       ? static_cast<UT>(UT{0} - static_cast<UT>(divisor))
  ...
  const unsigned sh =
      31u - detail::LeadingZeroCount32(static_cast<uint32_t>(abs_d - 1u));
...
HWY_INLINE constexpr DivisorParamsS<T> ComputeDivisorParams(T divisor) {
  ...
  params.multiplier =
      static_cast<int32_t>(static_cast<T>(static_cast<uint32_t>(tmp)));
  params.shift = static_cast<int>(sh);
  return params;
}
...
HWY_INLINE
    ...
    constexpr
#endif
    DivisorParamsS<T> ComputeDivisorParams(T divisor) {
  HWY_DASSERT(divisor != 0);
  if (HWY_UNLIKELY(divisor == 0)) {
    HWY_ABORT("intdiv: division by zero in ComputeDivisorParams");
  }
...
HWY_INLINE V IntDiv(D d, V dividend, const DivisorParamsU<T>& params) {
  ...
  if constexpr (sizeof(T) <= 2) {
    if constexpr (D::kPrivateLanes < 2) {
      ...
      const V t1 = OrderedDemote2To(d, ShiftRight<kShift>(prod_lo),
                                    ShiftRight<kShift>(prod_hi));
...
HWY_INLINE V IntDiv(D d, V dividend, const DivisorParamsS<T>& params) {
  ...
  if constexpr (sizeof(T) <= 2) {
    if constexpr (D::kPrivateLanes < 2) {
      ...
      const auto high = OrderedDemote2To(d, ShiftRight<kShift>(prod_lo),
                                         ShiftRight<kShift>(prod_hi));
...
HWY_INLINE void DivideArrayByScalar(T* HWY_RESTRICT array, size_t count,
                                    T divisor) {
  HWY_DASSERT(divisor != 0);
  if (HWY_UNLIKELY(divisor == 0)) HWY_ABORT("intdiv: division by zero");
...
template <typename T, HWY_IF_SIGNED(T)>
HWY_INLINE void FloorDivideArrayByScalar(T* HWY_RESTRICT array, size_t count,
                                         T divisor) {
  HWY_DASSERT(divisor != 0);
  if (HWY_UNLIKELY(divisor == 0)) HWY_ABORT("intdiv: division by zero");
