...
#define HWY_TARGET_INCLUDE "hwy/contrib/intdiv/intdiv_test.cc"  // NOLINT
// clang-format on
...
struct TestBasicDivision {
  ...
  void operator()(D d, size_t count, size_t misalign_a, RandomState& /*rng*/) {
    ...
    const T divisors[] = {T(1),   T(2),   T(3),   T(5),   T(7),   T(10),  T(11),
                          T(13),  T(16),  T(17),  T(25),  T(31),  T(32),  T(64),
                          T(100), T(127), T(128), T(255), T(256), T(1000)};
...
struct TestPowerOf2Division {
  ...
  void operator()(D d, size_t count, size_t misalign_a, RandomState& /*rng*/) {
    ...
    const int max_shift =
        static_cast<int>(sizeof(T) * 8 - hwy::IsSigned<T>()) - 1;
    ...
    for (int shift = 0; shift <= max_shift; ++shift) {
      ...
      for (size_t i = 0; i < count; ++i) {
        if constexpr (hwy::IsSigned<T>()) {
          a[i] =
              static_cast<T>(static_cast<int>(i) - static_cast<int>(count / 2));
        } else {
          a[i] = static_cast<T>(i);
        }
        expected[i] = static_cast<T>(a[i] / divisor);
...
struct TestSignedDivision {
  ...
  void operator()(D d, size_t count, size_t misalign_a, RandomState& /*rng*/) {
    ...
    const T divisors[] = {T(3),  T(5),  T(7),   T(-3),  T(-5),
                          T(-7), T(17), T(-17), T(100), T(-100)};
...
struct TestFloorDivision {
  ...
  void operator()(D d, size_t count, size_t misalign_a, RandomState& /*rng*/) {
    ...
    std::vector<T> divisors = {T(1), T(2),  T(3),  T(5),
                               T(7), T(11), T(17), T(100)};
    if constexpr (hwy::IsSigned<T>()) {
  ...
  void operator()(D d, size_t /*count*/, size_t /*misalign_a*/,
    ...
    if constexpr (sizeof(T) == 1) {
      ...
      if constexpr (hwy::IsSigned<T>()) {
        ...
        large_divisors = {T(65530), T(65531), T(65532),
                          T(65533), T(65534), T(65535)};
      }
    } else if constexpr (sizeof(T) == 4) {
      if constexpr (hwy::IsSigned<T>()) {
        const T kMin = std::numeric_limits<T>::min();
        const T kMax = std::numeric_limits<T>::max();
        large_divisors = {static_cast<T>(kMax - 2),
                          static_cast<T>(kMax - 1),
                          kMax,
                          static_cast<T>(kMin + 3),
    ...
    for (T divisor : large_divisors) {
      ...
      if constexpr (hwy::IsSigned<T>()) {
        boundary_dividends = {T(0), T(1), T(-1), std::numeric_limits<T>::max()};
...
void TestAllBasicDivision() {
  ForIntegerTypes(
      ForGE128Vectors<ForeachCountAndMisalign<TestBasicDivision>>());
}

void TestAllPowerOf2Division() {
  ForIntegerTypes(
      ForGE128Vectors<ForeachCountAndMisalign<TestPowerOf2Division>>());
}

void TestAllSignedDivision() {
  ForSignedTypes(
      ForGE128Vectors<ForeachCountAndMisalign<TestSignedDivision>>());
}

void TestAllFloorDivision() {
  ForIntegerTypes(
      ForGE128Vectors<ForeachCountAndMisalign<TestFloorDivision>>());
}
...
void TestAllConvenienceAPI() {
  ForIntegerTypes(
      ForGE128Vectors<ForeachCountAndMisalign<TestConvenienceAPI>>());
}
