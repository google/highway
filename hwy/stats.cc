...
HWY_DLLEXPORT void Stats::Assimilate(const Stats& other) {
  ...
  const double other_n = static_cast<double>(other.n_);
  const double product_n = n * other_n;
  const double n2 = n * n;
  const double other_n2 = other_n * other_n;
  const int64_t total_n2 = total_n * total_n;
  const double total_n3 =
      static_cast<double>(total_n2) * static_cast<double>(total_n);
  // Precompute reciprocal for speed - used at least twice.
  const double inv_total_n = 1.0 / static_cast<double>(total_n);
  const double inv_total_n2 = 1.0 / static_cast<double>(total_n2);
  ...
  m1_ = (n * m1_ + other_n * other.m1_) * inv_total_n;

  const double new_m2 = m2_ + other.m2_ + delta2 * product_n * inv_total_n;

  const double new_m3 =
      m3_ + other.m3_ + delta3 * product_n * (n - other_n) * inv_total_n2 +
      3.0 * delta * (n * other.m2_ - other_n * m2_) * inv_total_n;

  m4_ += other.m4_ +
         delta4 * product_n * (n2 - product_n + other_n2) / total_n3 +
         6.0 * delta2 * (n2 * other.m2_ + other_n2 * m2_) * inv_total_n2 +
         4.0 * delta * (n * other.m3_ - other_n * m3_) * inv_total_n;
